...
// Move classes outside to avoid re-creation on every mount/update
class CommonClass {
  constructor() {
    this.width = 0;
    this.height = 0;
  ...
  init(container) {
    ...
    try {
      ...
      console.error('Three.js WebGLRenderer failed to initialize:', e);
      return false;
    }
  }
  resize() {
...
class Advection extends ShaderPass {
  constructor(simProps, common) {
    ...
    super({ material, output: simProps.dst }, common);
    this.uniforms = this.props.material.uniforms;
    this.init();
  }
  init() {
...
class Simulation {
  ...
  createShaderPass() {
    ...
    this.viscous = new Viscous({ cellScale: this.cellScale, boundarySpace: this.boundarySpace, viscous: this.options.viscous, src: this.fbos.vel_1, dst: this.fbos.vel_viscous1, dst_: this.fbos.vel_viscous0, dt: this.options.dt }, this.common);
    this.divergence = new Divergence({ cellScale: this.cellScale, boundarySpace: this.boundarySpace, src: this.fbos.vel_viscous0, dst: this.fbos.div, dt: this.options.dt }, this.common);
    this.poisson = new Poisson({ cellScale: this.cellScale, boundarySpace: this.boundarySpace, src: this.fbos.div, dst: this.fbos.pressure_1, dst_: this.fbos.pressure_0 }, this.common);
    this.pressure = new PressurePass({ cellScale: this.cellScale, boundarySpace: this.boundarySpace, src_p: this.fbos.pressure_0, src_v: this.fbos.vel_viscous0, dst: this.fbos.vel_0, dt: this.options.dt }, this.common);
  }
  calcSize() {
...
export default function LiquidEther({
  ...
  resolution = 0.5, isBounce = false, colors = ['#1a75ff', '#00f2ff', '#000814'],
  style = {}, className = '', autoDemo = true, autoSpeed = 0.4, autoIntensity = 2.0,
  ...
  useEffect(() => {
    ...
    const webgl = new WebGLManager({
      $wrapper: mountRef.current, autoDemo, autoSpeed, autoIntensity, takeoverDuration, autoResumeDelay, autoRampDuration
    }, paletteTex, bgVec4);
    webglRef.current = webgl;

    const sim = webgl.output?.simulation;
    if (sim) Object.assign(sim.options, { mouseForce, cursorSize, isViscous, viscous, iterationsViscous, iterationsPoisson, dt, BFECC, resolution, isBounce });

    let raf;
    const loop = () => {
      if (isVisibleRef.current) webgl.render();
